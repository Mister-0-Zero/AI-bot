...
embedder = HuggingFaceEmbeddings(
    ...
    cache_folder="../models/embeding_model"
)
...
async def store_documents_async(file_text_data, persist_dir="../data/chroma"):
    ...
    try:
        ...
        logger.info(f"Документы добавлены в Chroma. Всего документов: {db._collection.count()}")
...
def load_vector_db(persist_dir="../data/chroma"):
    ...
    try:
        ...
        logger.info(f"База успешно загружена. Кол-во документов: {db._collection.count()}")
        return db
    except Exception as e:
