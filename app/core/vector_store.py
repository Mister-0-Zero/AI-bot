...
embedder = HuggingFaceEmbeddings(
    model_name="intfloat/multilingual-e5-base", cache_folder=str(EMBEDDING_DIR)
)

text_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200)


async def store_documents_async(file_text_data, persist_dir=CHROMA_DIR):
...
def load_vector_db(persist_dir=CHROMA_DIR):
    logger.info(f"Загрузка векторной базы из {persist_dir}")
    try:
        db = Chroma(persist_directory=str(persist_dir), embedding_function=embedder)
        logger.info(
